...
export default function AdminBlogPage() {
  ...
  const columns: DataTableColumn<BlogPost>[] = [
    { key: "title", header: "Title", cell: (item) => <span className="font-semibold text-[#082B6A]">{item.title}</span> },
    ...
    { key: "updated", header: "Updated", cell: (item) => item.updatedAt?.toLocaleDateString() ?? "-" },
    {
      ...
      cell: (item) => (
        <div className="flex gap-2">
          <button type="button" onClick={() => updateBlogPost(item.id, { title: item.title, slug: item.slug, excerpt: item.excerpt, content: item.content, image: item.image, published: !item.published })} className="border border-slate-300 px-3 text-xs font-bold uppercase tracking-[0.08em] text-[#082B6A]">
            {item.published ? "Unpublish" : "Publish"}
          </button>
          <button type="button" onClick={() => setEditing(item)} className="inline-flex size-9 items-center justify-center border border-slate-200 text-[#082B6A]" aria-label={`Edit ${item.title}`}>
