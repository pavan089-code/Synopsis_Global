    ...
    <section className="bg-slate-50 py-20 sm:py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <Reveal>
          <SectionHeading
            eyebrow="Process"
            title="Clear workflows for students, candidates, and employers"
          ...
          {processFlows.map((flow, flowIndex) => (
            <Reveal key={flow.title} delay={flowIndex * 0.08}>
              <article className="h-full border border-slate-200 bg-white p-7 shadow-sm">
                <h3 className="text-2xl font-semibold text-[#082B6A]">{flow.title}</h3>
                ...
                <ol className="mt-8 grid gap-5">
                  {flow.steps.map((step, index) => (
                    <li key={step} className="grid grid-cols-[3rem_1fr] items-center gap-4">
                      <span className="flex size-12 items-center justify-center rounded-sm bg-[#082B6A] text-sm font-bold text-[#C9A227]">
